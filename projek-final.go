...
func main() {
	...
	for {
		...
		case "4": //mencari Data berdasarkan nama atau biaya langganan
			...
			cetakUI()
			fmt.Scan(&perintah)
		case "5": //untuk mengedit data indeks tertentu
			cetakArr(arrData, nData)
			editDataNama(&arrData, nData)
			cetakArr(arrData, nData)
			cetakUI()
			fmt.Scan(&perintah)
		case "6": //menampilkan detail semua langganan dan pengeluaran
...
func cetakArr(A finansialManage, n int) {
	...
	var i int
	fmt.Println("==========================================")
	fmt.Printf("%s \t%s \t%s\n", "No.", "Nama Langganan", "  Biaya")
	fmt.Println("==========================================")
	for i = 0; i < n; i++ {
		fmt.Printf("%d.\t%s \t%s %d\n", i+1, A[i].namaLangganan, "Rp.", A[i].biaya)
	}
	fmt.Println("==========================================")
}
